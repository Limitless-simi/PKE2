...
// using ControllerSelection; // Add this line
...
public class MultiStepCanvasInput : MonoBehaviour
{
    ...
    public void Answer(AudioClip clip = null)
    {
        ...
        if ((isCorrect && steps[currentIndex].correctAnswer.Split(';').Length - 3 <= correctAnswersCount) || !steps[currentIndex].mustAnswerCorrectly || possibleTries == 0)
        {
            ...
            if (toActivate != null)
            {
                ...
                // Update XR Ray Interactor (assuming your toActivate object has XR Ray Interactor component)
                XRBaseInteractor xrInteractor = toActivate.GetComponent<XRBaseInteractor>();
                if (xrInteractor != null) xrInteractor.enabled = true;
            ...
            if (currentIndex >= steps.Length || steps[currentIndex - 1].ShouldStopAtThis)
            {
                ...
                // replacement
                // Disable XR Ray Interactor (assuming your obj's parent has XR Ray Interactor component)
                XRBaseInteractor xrInteractor = obj.transform.parent.gameObject.GetComponent<XRBaseInteractor>();
                if (xrInteractor != null) xrInteractor.enabled = false;
    ...
    {
        // if has UnityEngine.XR.Interaction.Toolkit.OVRRayInteractor comp, enable it
        // OVRRayInteractor rayInteractor = toActivate.GetComponent<OVRRayInteractor>();
